...
| **Bulk edit subscriptions** (filter → count → confirm → queued run) | [30-subscriptions.md § Part C](30-subscriptions.md) | built | built | `app/Domain/Bulk`. The target is a stored `SubscriptionCriteria` (a filter, never a list of ids), so the preview that counts and the worker that walks read ONE definition — sized for tens of thousands of rows. Five whitelisted verbs (set date · shift date · re-cadence · pause · resume); **no cancel** — terminal, emails everybody, no way back. Each verb's `eligible()` mirrors what the detail page allows; `status` is never written set-based (lifecycle verbs go row-by-row through `transitionTo()`). `BulkEditRunner` commits rows + audit + cursor in one transaction per chunk, so a resumed or retried run never double-applies the non-idempotent shift. No apply without a preview, the typed count is checked against a recount, and a due-now date or a backwards shift needs an explicit acknowledgement. |
| **Refund / cancel drawer** (full, partial, cancel-the-order, restock) | [refunds plan](../plans/refunds-cancellations-and-card-update-links.md) | built | built | `RefundRequest` + `RefundOrchestrator`: money (PayPlus) → store (`StoreRefunder` per platform) → credit note (`DocumentContext::REFUND`/`CANCELLATION`). Opened from the payment page + payments list; the plan's ledger table links through. A store leg that fails after money moved = `needs_attention` + retry. Orders this app never charged (Shopify Payments, a Woo order paid on another gateway) refund through the STORE — there the store call IS the refund, so its failure is `failed`, not `needs_attention`. Refunds pressed inside **WooCommerce** are mirrored back (the gateway's `process_refund` runs the money leg; other refunds record a delta against a running total, so a duplicate hook counts nothing), and the plugin adds its OWN refund box to the order screen — the one path that works for deposit/instalment orders, which WooCommerce can only "refund manually". Shopify's `refunds/create` mirror awaits protected-data approval. |
