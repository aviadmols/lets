...
{{-- Default upcoming-charge reminder. Trusted platform view (Blade-safe). --}}
@section('body')
    <div dir="rtl" style="font-family:Arial,Helvetica,sans-serif;max-width:560px;margin:0 auto;padding:24px;color:#1f2937;background:#ffffff;border-radius:12px;border:1px solid #e5e7eb;">
        ...
        <p style="font-size:15px;line-height:1.6;margin:0 0 14px;">זוהי תזכורת שהחיוב הבא עבור <strong>{{ $product_title }}</strong> צפוי בתאריך <strong>{{ $next_charge_date }}</strong>.</p>
        <p style="font-size:15px;line-height:1.6;margin:0 0 14px;font-weight:700;">סכום: {{ $amount }} {{ $currency }}</p>
        @if (! empty($portal_url))
            <a href="{{ $portal_url }}" style="display:inline-block;background:#111827;color:#ffffff;text-decoration:none;padding:12px 22px;border-radius:8px;font-size:15px;font-weight:600;margin:8px 0 16px;">ניהול המנוי שלי</a>
        @endif
