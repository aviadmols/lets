...
@section('body')
    <div dir="rtl" style="font-family:Arial,Helvetica,sans-serif;max-width:560px;margin:0 auto;padding:24px;color:#1f2937;background:#ffffff;border-radius:12px;border:1px solid #e5e7eb;">
        ...
        {{-- $installment_progress is the whole "(payment X of Y)" aside, empty for an
             open-ended subscription. It is a VAR and not an @if because the token
             template this view mirrors is substituted with strtr, long after Blade. --}}
        <p style="font-size:15px;line-height:1.6;margin:0 0 14px;">התשלום עבור <strong>{{ $product_title }}</strong> התקבל בהצלחה{{ $installment_progress ?? '' }}.</p>
